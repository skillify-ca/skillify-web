...
const BlogPost = ({ date, title, image, description, link, color }) => {
  ...
  return (
    (<Link href={link} legacyBehavior>
      <div
        className={`flex flex-col-reverse sm:flex-row group p-2 justify-between items-center border-t-0 hover:bg-backgroundHover transition-all transform text-textPrimary bg-backgroundPrimary shadow-sm cursor-pointer overflow-hidden ${getBorderColour()}`}
      >
