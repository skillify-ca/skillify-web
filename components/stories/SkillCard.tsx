...
const SkillCard = ({ loading, userSkillData, skill }) => {
  ...
  return <Link href={`/practice/${skill.id}`}><div
  ...
  >
    ...
    <div className="col-span-2 bg-white h-full rounded-r-lg">
      <p className={` col-span-2 justify-center rounded-r-lg text-center flex items-center h-full text-4xl mb-4 transform transition-all ease-in-out duration-200 ${isSelected ? "-rotate-45 scale-110" : ""}`}>
        {!loading &&
          userSkillData &&
    ...
    {/* <img
      className="object-cover col-span-2 rounded-l-xl"
      src={
        !skill.published
          ? "/images/skills/lock.png"
          : `https://placeimg.com/640/480/tech`
    ...
    <p className="text-center p-4 flex col-span-8 bg-gray-200 items-center justify-center">
      {`I can ${skill.description}`}
    </p>
    <p className={`bg-green-300 col-span-2 justify-center text-center flex items-center h-full text-4xl mb-4 transform transition-all ease-in-out duration-200 ${isSelected ? "-rotate-45 scale-110" : ""}`}>
      {!loading &&
        data &&
