...
import { getLessonForHTMLCourse } from "../../../api/studentPortal/courses/html";
...
const LessonPage = ({ lessonComponents, nextSlug }: ResponseData) => {
  return (
    <div className="grid max-w-5xl grid-cols-1 gap-8 p-4 pb-16 mx-auto md:p-8 lg:p-12">
      <ProgressBar
        completed={100}
        exitLink="/studentPortal/courses/html"
      />
      ...
      {nextSlug && (
        <Link scroll href={`/studentPortal/courses/html/${nextSlug}`}>
...
export async function getServerSideProps({ params }) {
  ...
  return {
    props: getLessonForHTMLCourse(slug),
  };
}
