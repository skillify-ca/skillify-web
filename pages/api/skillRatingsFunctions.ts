...
export const transformSkillsAndRatings = (skillRatings: SkillsAndRatings[]) => {
  ...
  return skillRatings.map((row) => {
    return {
      ...
      studentRating:
        row.intro_course_skills_users_aggregate.nodes.length > 0
          ? parseInt(
              row.intro_course_skills_users_aggregate.nodes[0].studentRating
            )
          : 0,
      userSkillId: row.intro_course_skills_users[0]?.id
        ? row.intro_course_skills_users[0].id
        : Math.random(),
    };
  });
};

export const transformSkillRatingForDB = (
