const MULTIPLICATION_TWO = {
  title: "Two Times Table",
  id: 2,
  answer: "2,4,8,3,7,5,0,6,9,1",
  questions: [
    {
      parts: [{ text: "2 x 6 = 1" }, { tileData: { type: "tile", index: 0 } }],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 1 } },
        { text: " x 2 = " },
        { tileData: { type: "tile", index: 2 } },
      ],
    },
    {
      parts: [
        { text: "2 x " },
        { tileData: { type: "tile", index: 3 } },
        { text: " = 6" },
      ],
    },
    {
      parts: [{ tileData: { type: "tile", index: 4 } }, { text: " x 2 = 14" }],
    },
    {
      parts: [
        { text: "2 x " },
        { tileData: { type: "tile", index: 5 } },
        { text: " = 1" },
        { tileData: { type: "tile", index: 6 } },
      ],
    },
    {
      parts: [{ text: "2 x 8 = 1" }, { tileData: { type: "tile", index: 7 } }],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 8 } },
        { text: " x 2 = " },
        { tileData: { type: "tile", index: 9 } },
        { text: "8" },
      ],
    },
  ],
};

const MULTIPLICATION_THREE = {
  title: "Three Times Table",
  id: 3,
  answer: "5,4,9,7,2,1,6,8,3,0",
  questions: [
    {
      parts: [{ text: "5 x 3 = 1" }, { tileData: { type: "tile", index: 0 } }],
    },
    {
      parts: [{ text: "8 x 3 = 2" }, { tileData: { type: "tile", index: 1 } }],
    },
    {
      parts: [
        { text: "3 x " },
        { tileData: { type: "tile", index: 2 } },
        { text: " = 2" },
        { tileData: { type: "tile", index: 3 } },
      ],
    },
    {
      parts: [
        { text: "3 x 7 = " },
        { tileData: { type: "tile", index: 4 } },
        { tileData: { type: "tile", index: 5 } },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 6 } },
        { text: " x 3 = 1" },
        { tileData: { type: "tile", index: 7 } },
      ],
    },
    {
      parts: [
        { text: "10 x 3 = " },
        { tileData: { type: "tile", index: 8 } },
        { tileData: { type: "tile", index: 9 } },
      ],
    },
  ],
};

const MULTIPLICATION_FOUR = {
  title: "Four Times Table",
  id: 4,
  answer: "7,1,3,2,8,4,9,6,5,0",
  questions: [
    {
      parts: [
        { text: "4 x " },
        { tileData: { type: "tile", index: 0 } },
        { text: " = 28" },
      ],
    },
    {
      parts: [
        { text: "4 x 4 = " },
        { tileData: { type: "tile", index: 1 } },
        { text: "6" },
      ],
    },
    {
      parts: [
        { text: "4 x " },
        { tileData: { type: "tile", index: 2 } },
        { text: " = 1" },
        { tileData: { type: "tile", index: 3 } },
      ],
    },
    {
      parts: [{ tileData: { type: "tile", index: 4 } }, { text: " x 4 = 32" }],
    },
    {
      parts: [{ text: "6 x 4 = 2" }, { tileData: { type: "tile", index: 5 } }],
    },
    {
      parts: [
        { text: "4 x " },
        { tileData: { type: "tile", index: 6 } },
        { text: " = 3" },
        { tileData: { type: "tile", index: 7 } },
      ],
    },
    {
      parts: [
        { text: "4 x " },
        { tileData: { type: "tile", index: 8 } },
        { text: " = 2" },
        { tileData: { type: "tile", index: 9 } },
      ],
    },
  ],
};
const MULTIPLICATION_FIVE = {
  title: "Five Times Table",
  id: 5,
  answer: "1, 8, 0, 2, 9, 6, 3, 4, 7, 5",
  questions: [
    {
      parts: [
        { text: "5 x 3 = " },
        { tileData: { type: "tile", index: 0 } },
        { text: "5" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 1 } },
        { text: " x 5 = 4" },
        { tileData: { type: "tile", index: 2 } },
      ],
    },
    {
      parts: [
        { text: "5 x 5 = " },
        { tileData: { type: "tile", index: 3 } },
        { text: "5" },
      ],
    },
    {
      parts: [{ tileData: { type: "tile", index: 4 } }, { text: " x 5 = 45" }],
    },
    {
      parts: [
        { text: "5 x " },
        { tileData: { type: "tile", index: 5 } },
        { text: " = " },
        { tileData: { type: "tile", index: 6 } },
        { text: "0" },
      ],
    },
    {
      parts: [
        { text: "5 x " },
        { tileData: { type: "tile", index: 7 } },
        { text: " = 20" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 8 } },
        { text: " x 5 = 3" },
        { tileData: { type: "tile", index: 9 } },
      ],
    },
  ],
};

const MULTIPLICATION_SIX = {
  title: "Six Times Table",
  id: 6,
  answer: "2, 3, 1, 7, 9, 5, 6, 4, 8, 0",
  questions: [
    {
      parts: [
        { text: "6 x " },
        { tileData: { type: "tile", index: 0 } },
        { text: " = 12" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 1 } },
        { text: " x 6 = " },
        { tileData: { type: "tile", index: 2 } },
        { text: "8" },
      ],
    },
    {
      parts: [{ tileData: { type: "tile", index: 3 } }, { text: " x 6 = 42" }],
    },
    {
      parts: [{ tileData: { type: "tile", index: 4 } }, { text: " x 6 = 24" }],
    },
    {
      parts: [
        { text: "6 x " },
        { tileData: { type: "tile", index: 5 } },
        { text: " = " },
        { tileData: { type: "tile", index: 6 } },
        { text: "4" },
      ],
    },
    {
      parts: [{ text: "6 x 6 = 3" }, { tileData: { type: "tile", index: 7 } }],
    },
    {
      parts: [{ text: "6 x 8 = 4" }, { tileData: { type: "tile", index: 8 } }],
    },
    {
      parts: [{ text: "6 x 5 = 3" }, { tileData: { type: "tile", index: 9 } }],
    },
  ],
};
const MULTIPLICATION_SEVEN = {
  title: "Seven Times Table",
  id: 7,
  answer: "7, 4, 8, 9, 6, 2, 0, 3, 1, 5",
  questions: [
    {
      parts: [
        { text: "7 x " },
        { tileData: { type: "tile", index: 0 } },
        { text: " = 49" },
      ],
    },
    {
      parts: [{ text: "7 x 2 = 1" }, { tileData: { type: "tile", index: 1 } }],
    },
    {
      parts: [{ text: "7 x 4 = 2" }, { tileData: { type: "tile", index: 2 } }],
    },
    {
      parts: [{ tileData: { type: "tile", index: 3 } }, { text: " x 7 = 63" }],
    },
    {
      parts: [
        { text: "7 x " },
        { tileData: { type: "tile", index: 4 } },
        { text: " = 4" },
        { tileData: { type: "tile", index: 5 } },
      ],
    },
    {
      parts: [{ text: "7 x 10 = 7" }, { tileData: { type: "tile", index: 6 } }],
    },
    {
      parts: [
        { text: "7 x " },
        { tileData: { type: "tile", index: 7 } },
        { text: " = 2" },
        { tileData: { type: "tile", index: 8 } },
      ],
    },
    {
      parts: [{ text: "5 x 7 = 3" }, { tileData: { type: "tile", index: 9 } }],
    },
  ],
};

const MULTIPLICATION_EIGHT = {
  title: "Eight Times Table",
  id: 8,
  answer: "3,5,0,4,8,9,2,1,7,6",
  questions: [
    {
      parts: [
        { text: "8 x 4 = " },
        { tileData: { type: "tile", index: 0 } },
        { text: "2" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 1 } },
        { text: " x 8 = 4" },
        { tileData: { type: "tile", index: 2 } },
      ],
    },
    {
      parts: [
        { text: "8 x 6 = " },
        { tileData: { type: "tile", index: 3 } },
        { text: "8" },
      ],
    },
    {
      parts: [{ tileData: { type: "tile", index: 4 } }, { text: " x 8 = 64" }],
    },
    {
      parts: [
        { text: "8 x " },
        { tileData: { type: "tile", index: 5 } },
        { text: " = 7" },
        { tileData: { type: "tile", index: 6 } },
      ],
    },
    {
      parts: [
        { text: "2 x 8 = " },
        { tileData: { type: "tile", index: 7 } },
        { text: "6" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 8 } },
        { text: " x 8 = 5" },
        { tileData: { type: "tile", index: 9 } },
      ],
    },
  ],
};

const MULTIPLICATION_NINE = {
  title: "Nine Times Table",
  id: 9,
  answer: "5, 0, 6, 4, 9, 1, 3, 8, 7, 2",
  questions: [
    {
      parts: [{ text: "9 x 5 = 4" }, { tileData: { type: "tile", index: 0 } }],
    },
    {
      parts: [{ text: "9 x 10 = 9" }, { tileData: { type: "tile", index: 1 } }],
    },
    {
      parts: [{ tileData: { type: "tile", index: 2 } }, { text: " x 9 = 54" }],
    },
    {
      parts: [
        { text: "9 x " },
        { tileData: { type: "tile", index: 3 } },
        { text: " = 36" },
      ],
    },
    {
      parts: [
        { tileData: { type: "tile", index: 4 } },
        { text: " x 9 = 8" },
        { tileData: { type: "tile", index: 5 } },
      ],
    },
    {
      parts: [{ text: "7 x 9 = 6" }, { tileData: { type: "tile", index: 6 } }],
    },
    {
      parts: [{ text: "2 x 9 = 1" }, { tileData: { type: "tile", index: 7 } }],
    },
    {
      parts: [
        { text: "9 x 8" },
        { tileData: { type: "tile", index: 8 } },
        { tileData: { type: "tile", index: 9 } },
      ],
    },
  ],
};

export const PUZZLE_DATA = {
  "2": MULTIPLICATION_TWO,
  "3": MULTIPLICATION_THREE,
  "4": MULTIPLICATION_FOUR,
  "5": MULTIPLICATION_FIVE,
  "6": MULTIPLICATION_SIX,
  "7": MULTIPLICATION_SEVEN,
  "8": MULTIPLICATION_EIGHT,
  "9": MULTIPLICATION_NINE,
};

